    ...
    <>
      <div className="bg-lightGray pt-12">
        <div className="container">
          <div className="flex flex-row justify-between items-center mb-4 border-b-[1px] border-[#CBCBCB]">
      ...
      >
        <div className="grid grid-cols-4 gap-5">
          {ticket.matches.data.map((jobSeeker, index) => (
            <Card
              key={index}
              jobSeeker={jobSeeker.attributes}
...
export const getServerSideProps = async ({ params }) => {
  const response = await Axios.get(`tickets/${params.id}?populate=deep`);
  return {
    props: {
